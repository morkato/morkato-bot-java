  ...
  public void setup(Extension extension)
    ...
    try {
      ...
      for (Map.Entry<String, Class<? extends Command<?>>> entry : commandNames.entrySet()) {
        ...
        Consumer<Object> consumer = registration.create(NamePointer.class);
        if (Objects.isNull(consumer)) {
          logger.warn("Ignoring name pointer: {} for command: {}. Registration not registered a NamePointer. Ignoring.", name, clazz.getName());
          return;
        }
